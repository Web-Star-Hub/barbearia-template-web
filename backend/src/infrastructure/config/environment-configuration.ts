import { getEnvironmentFilePathUsed } from './load-environment';
...
function normalizeStringValue(rawValue: string | undefined): string {
...
function getRequiredEnvironmentVariable(variableName: string): string {
    ...
    if (!normalizedValue) {
        const environmentFileHint = getEnvironmentFilePathUsed()
            ? ` (arquivo carregado: ${getEnvironmentFilePathUsed()})`
            : '';

        throw new Error(
            `Variável de ambiente obrigatória ausente ou vazia: ${variableName}${environmentFileHint}. ` +
...
function getNumberEnvironmentVariable(
    ...
    if (Number.isNaN(convertedValue)) {
        throw new Error(
            `A variável de ambiente ${variableName} deve ser um número válido. Valor recebido: ${rawValue}`
        );
    }
...
function getBooleanEnvironmentVariable(
    ...
    throw new Error(
        `A variável de ambiente ${variableName} deve ser true ou false. Valor recebido: ${rawValue}`
    );
}
...
function parseCorsAllowedOrigins(): string[] {
    ...
    if (
        allowedOriginsRawValue === undefined ||
    ...
    if (originsList.length === 0) {
        throw new Error(
            'CORS_ALLOWED_ORIGINS está definida mas não contém nenhuma origem válida após o parsing. ' +
...
function validateMongoDbConnectionString(connectionString: string): void {
    const trimmedConnectionString = connectionString.trim();

    if (!/^mongodb(\+srv)?:\/\//i.test(trimmedConnectionString)) {
        throw new Error(
            'MONGODB_URI inválida: deve começar com mongodb:// ou mongodb+srv://. ' +
                'Verifique se a URI está completa e sem caracteres estranhos no início.'
        );
    }
}

function validateJwtSecret(secretValue: string): void {
    if (secretValue.length < 8) {
        throw new Error(
            'JWT_SECRET inválido: use pelo menos 8 caracteres por segurança.'
        );
    }
}

function buildEnvironmentConfiguration(): EnvironmentConfiguration {
    ...
    const jwtExpirationTime = getOptionalEnvironmentVariable(
        'JWT_EXPIRATION_TIME',
        '1d'
    );
    ...
    if (!jwtExpirationTime.trim()) {
        throw new Error(
            'JWT_EXPIRATION_TIME não pode ser uma string vazia. Use um valor como 1d, 12h ou um número em segundos.'
        );
    }
    ...
    const businessName = getOptionalEnvironmentVariable(
        'BUSINESS_NAME',
        'Barbearia'
    );
    const defaultSlotDurationMinutes = getNumberEnvironmentVariable(
